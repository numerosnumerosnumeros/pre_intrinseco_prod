import { getFilterWords, goodWords } from './cleaner_consts.js'
...
export function cleanChunk(target, chunk, language, period) {
  ...
  if (
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (containsJanuary) {
        // Process January section (equivalent to process_january_section)
  ...
  // Convert accounting notation (parentheses) to minus sign
...
function checkCurrencyMillionsOrUds(text, position) {
  ...
  for (const symbol of currencySymbols) {
    ...
    const millionWords = ['million', 'millions', 'millón', 'millon', 'millones']
